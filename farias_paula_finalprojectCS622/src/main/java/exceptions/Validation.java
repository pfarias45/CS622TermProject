...
public abstract class Validation extends ReadScript {
	...
	*/
	public static boolean validateArrSize(String script, ArrayList<String> inputList) {
		try {
			...
			if(script.equals(titleEmpScript) || script.equals(contactScript) || script.equals(degSchoScript)) numItems = 2;
	...
	*/
	...
	/**
	 * Desc: Method validates that input is either Y or N
	 * Param: String input
	 * Return: Boolean if valid
	*/
	public static boolean validateYN(String input) {
		try {
			...
			else throw new InvalidEntryException(errorScript + "invalid entry. Please put enter either Y or N.");
		}
	 ...
	 * Return: Boolean if valid
	*/
	public static boolean validatePswrd(Account<Admin> user, String input) {
		try {
			if(input.equals(user.getAccount().getPswd())) return true;
	...
	*/
	...
	/**
	 * Desc: Method validates user selection of email, download or both
	 * Param: String input
	 * Return: Boolean if valid
	*/
	public static boolean validatePDFPath(String c) {
		try {
			String choice = c.toUpperCase();
			if(choice.equals("B") || choice.equals("E") || choice.equals("D")) return true;
			else throw new InvalidEntryException(errorScript + "invalid entry. Please put enter either B, E, or D.");	
		}
		catch (InvalidEntryException e) {
			System.out.println(e.getMessage());
			return false;	
